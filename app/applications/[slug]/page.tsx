...
export async function generateMetadata({ params }: PageProps<"/applications/[slug]">): Promise<Metadata> { const { slug } = await params; const facet = getFacetBySlug("application", slug); return facet ? { title: `${facet.value} Products`, description: `Browse ${facet.count} products listed for ${facet.value} in the supplied catalogue.`, alternates: { canonical: `/applications/${slug}` } } : {}; }
