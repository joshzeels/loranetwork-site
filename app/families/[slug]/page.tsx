...
export default async function FamilyPage({ params }: PageProps<"/families/[slug]">) {
  ...
  return <>
    ...
    <section className="page-hero"><div className="shell"><nav className="breadcrumbs"><Link href="/">Home</Link><span>/</span><Link href="/families">Product families</Link><span>/</span><span>{family.name}</span></nav><div className="facet-detail-heading"><div><h1>{family.name}</h1></div><p>{family.purpose}</p></div></div></section>
    <section className="section shell answer-section"><div><h2>Which {family.name} models are available?</h2><p className="lead-copy">LoRa Network lists {familyProducts.length} models in the {family.name} family. Compare their specifications, interfaces, package information and prices below.</p></div><aside className="evidence-note"><strong>About this product family</strong><p>{family.purpose}</p></aside></section>
