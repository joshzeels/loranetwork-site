...
export function EnquiryBuilder({ initialSku }: { initialSku: string }) {
  ...
  return <div className="enquiry-builder"><form onSubmit={submit} onFocusCapture={() => { if (!startedAt.current) startedAt.current = Date.now(); }} className="enquiry-form" noValidate>
    <div className="form-row"><label htmlFor="name">Name<span aria-hidden="true"> *</span><input id="name" name="name" required maxLength={100} autoComplete="name" {...a11y("name")} />{error("name")}</label><label htmlFor="company">Company<input id="company" name="company" maxLength={120} autoComplete="organization" {...a11y("company")} />{error("company")}</label></div>
    <div className="form-row"><label htmlFor="email">Email<span aria-hidden="true"> *</span><input id="email" name="email" type="email" required maxLength={254} autoComplete="email" {...a11y("email")} />{error("email")}</label><label htmlFor="phone">Phone<input id="phone" name="phone" type="tel" maxLength={40} autoComplete="tel" {...a11y("phone")} />{error("phone")}</label></div>
    ...
    <label htmlFor="message">Message<span aria-hidden="true"> *</span><textarea id="message" name="message" rows={6} required maxLength={3000} {...a11y("message")} />{error("message")}</label>
