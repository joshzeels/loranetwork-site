...
export function FacetDetail({ facet, products, basePath, label, kind }: { facet: CatalogueFacet; products: PublicDraginoProduct[]; basePath: string; label: string; kind: "application" | "iotInterface" }) {
  ...
  return <>
    ...
    {(applications.length || interfaces.length || guides.length || families.length) ? <section className="section shell"><div className="section-heading"><h2>Continue your product research</h2></div><div className="link-cluster">{applications.map((item) => <Link href={`/applications/${item.slug}`} key={`a-${item.slug}`}>{item.value}</Link>)}{interfaces.map((item) => <Link href={`/connectivity/${item.slug}`} key={`i-${item.slug}`}>{item.value}</Link>)}{families.map((family) => <Link href={`/families/${family.slug}`} key={`f-${family.slug}`}>{family.name} family</Link>)}{guides.map((guide) => <Link href={`/guides/${guide.slug}`} key={`g-${guide.slug}`}>{guide.title}</Link>)}</div></section> : null}
